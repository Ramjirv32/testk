...
export default function MBTIAdminPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            if (activeTab !== 'results') {
                const response = await fetch(`${API_URL}/api/admin/mbti/registrations?status=${activeTab}`, {
                    headers: {
                ...
                const response = await fetch(`${API_URL}/api/admin/test-results?test_type=mvti`, {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                });
    ...
    const fetchStats = async () => {
        try {
            const [pendingRes, approvedRes, rejectedRes, resultsRes] = await Promise.all([
                fetch(`${API_URL}/api/admin/mbti/registrations?status=pending`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                }),
                fetch(`${API_URL}/api/admin/mbti/registrations?status=approved`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                }),
                fetch(`${API_URL}/api/admin/mbti/registrations?status=rejected`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                }),
                fetch(`${API_URL}/api/admin/test-results?test_type=mvti`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                })
            ]);
    ...
    const handleApprove = async (id: string) => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/admin/mbti/approve/${id}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
    ...
    const handleReject = async (id: string) => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/admin/mbti/reject/${id}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            <div className="card border-0 shadow-sm mb-4" style={{
                ...
                <div className="card-body p-3">
                    <div className="d-flex justify-content-between align-items-center">
                        <div>
                            <h4 className="mb-1" style={{ fontSize: '20px', fontWeight: '600' }}>MBTI Test Approvals</h4>
                            <p className="mb-0 opacity-75" style={{ fontSize: '13px' }}>Manage user registrations for MBTI personality test</p>
                        </div>
                        <div style={{ fontSize: '40px', opacity: 0.3 }}>
                            <i className="fas fa-person-circle"></i>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <ul className="nav nav-tabs border-bottom" style={{ padding: '0 20px' }}>
                        <li className="nav-item">
                            <button
                                ...
                                style={{
                                    color: activeTab === 'pending' ? '#dc2626' : '#666',
                                    borderBottom: activeTab === 'pending' ? '3px solid #dc2626' : 'none',
                                                ...
                                                {results.length === 0 ? (
                                                    ...
                                                    results.map((result) => (
                                                        <tr key={result.id}>
                                                            ...
                                                            <td>
                                                                <Link
                                                                    href={`/admin/mbti/result/${result.id}`}
                                                                    className="btn btn-sm"
                                                                    style={{
