...
export default function CognitiveRegisterPage() {
    ...
    useTestRegistrationWebSocket({
        ...
        onRegistrationApproved: (data) => {
            console.log(' Cognitive Registration approved:', data);
            setRegistrationStatus({
                status: 'approved',
                created_at: registrationStatus?.created_at || new Date().toISOString(),
            });

            alert(' Your Cognitive test registration has been approved! You can now take the test.');
        },
        onRegistrationRejected: (data) => {
            console.log(' Cognitive Registration rejected:', data);
            setRegistrationStatus({
                status: 'rejected',
                reason: data.reason,
                created_at: registrationStatus?.created_at || new Date().toISOString(),
            ...
            alert(` Your Cognitive test registration has been rejected. Reason: ${data.reason}`);
        },
    });
    ...
    const checkRegistrationStatus = async () => {
        try {
            const response = await fetch(`${API_URL}/api/cognitive/registration`, {
                headers: {
    ...
    const handleRegister = async () => {
        setSubmitting(true);
        try {
            const response = await fetch(`${API_URL}/api/cognitive/register`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                ...
                >
                    ...
                    <h1 style={{ fontSize: '28px', fontWeight: 'bold', color: '#1f2937', marginBottom: '12px', textAlign: 'center' }}>
                        Cognitive Test Registration
                    </h1>

                    <p style={{ fontSize: '16px', color: '#6b7280', marginBottom: '32px', textAlign: 'center' }}>
                        Register for the Cognitive abilities test. Your registration will be reviewed by an admin before you can proceed.
                    </p>
                        ...
                        <>
                            <div style={{
                                ...
                                <ul style={{ fontSize: '13px', color: '#1e40af', margin: '0', paddingLeft: '20px' }}>
                                    ...
                                    <li>Tests cognitive and reasoning abilities</li>
                                    <li>Takes approximately 45-60 minutes</li>
                                    <li>For students age 16-17</li>
                                </ul>
                            </div>
