...
export default function PsychometricRegisterPage() {
    ...
    useTestRegistrationWebSocket({
        ...
        onRegistrationApproved: (data) => {
            console.log(' Psychometric Registration approved:', data);
            setRegistrationStatus({
                status: 'approved',
                created_at: registrationStatus?.created_at || new Date().toISOString(),
            });

            alert(' Your Psychometric test registration has been approved! You can now take the test.');
        },
        onRegistrationRejected: (data) => {
            console.log(' Psychometric Registration rejected:', data);
            setRegistrationStatus({
                status: 'rejected',
                reason: data.reason,
                created_at: registrationStatus?.created_at || new Date().toISOString(),
            ...
            alert(` Your Psychometric test registration has been rejected. Reason: ${data.reason}`);
        },
    });
    ...
    const checkRegistrationStatus = async () => {
        try {
            const response = await fetch(`${API_URL}/api/psychometric/registration`, {
                headers: {
    ...
    const handleRegister = async () => {
        setSubmitting(true);
        try {
            const response = await fetch(`${API_URL}/api/psychometric/register`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                ...
                >
                    ...
                    <p style={{ fontSize: '16px', color: '#6b7280', marginBottom: '32px', textAlign: 'center' }}>
                        Register for the Psychometric assessment test. Your registration will be reviewed by an admin before you can proceed.
                    </p>
                    ...
                    {registrationStatus ? (
                        ...
                        >
                            ...
                            {registrationStatus.status === 'approved' && (
                                <button
                                    onClick={() => router.push('/user-dashboard/psychometric-test')}
                                    style={{
                        ...
                        <>
                            <div style={{
                                ...
                                <ul style={{ fontSize: '13px', color: '#1e40af', margin: '0', paddingLeft: '20px' }}>
                                    <li>15 questions</li>
                                    <li>Psychometric development assessment</li>
                                    <li>Takes approximately 10-15 minutes</li>
                                    <li>For students age 15 and below</li>
                                </ul>
                            </div>
